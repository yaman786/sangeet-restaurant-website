...
async function testAutoLogout() {
  console.log("Starting Auto-Logout QA Test ONLINE against Vercel...");
  const browser = await puppeteer.launch({ headless: 'new', args: ['--no-sandbox'] });
  const page = await browser.newPage();
  
  try {
    // 1. Go to frontend, inject bad token
    await page.goto('https://sangeet-restaurant-testing-frontend.vercel.app', { waitUntil: 'networkidle2' });
    await page.evaluate(() => {
    ...
    await page.goto('https://sangeet-restaurant-testing-frontend.vercel.app/admin/reservations', { waitUntil: 'networkidle2' });
