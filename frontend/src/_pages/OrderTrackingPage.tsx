...
const OrderTrackingPage = () => {
  ...
  const [reviewedOrders, setReviewedOrders] = useState<Set<any>>(new Set());
  ...
  useEffect(() => {
    ...
    const handleStatusUpdate = (data: any) => {
      ...
      if (data.orderId && orderId && 
        ...
        if ((statusMessages as any)[data.status]) {
  ...
  const getTotalAmount = () => {
    ...
    return order.items.reduce((total: any, item: any) => total + parseFloat(item.total_price), 0);
  };
  ...
  const currentStatus = (orderStatuses as any)[order.status] || orderStatuses.pending;
  ...
  return (
    <div className="min-h-screen bg-sangeet-neutral-950 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        {(order.status === 'ready' || order.status === 'completed') && !reviewedOrders.has(order.id) && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="text-center">
              ...
              <h3 className="text-lg font-semibold text-sangeet-400 mb-2">
      ...
      <ReviewModal
        ...
        onReviewSubmitted={() => {
          if (reviewModal.order) {
            setReviewedOrders(prev => new Set(Array.from(prev).concat(reviewModal.order.id)));
          }
