...
const UnifiedOrderPage = () => {
  ...
  const [reviewedOrders, setReviewedOrders] = useState<Set<number | string>>(new Set());
    ...
    if (orderId) {
      ...
      const handleOrderUpdate = (updateData: any) => {
        ...
        if (updateData.orderId && orderId && 
          ...
          if ((statusMessages as any)[updateData.status]) {
            if (updateData.status === 'cancelled') {
              ...
              toast.success((statusMessages as any)[updateData.status]);
            }
          }
        }
      };
  ...
  const getTotalAmount = () => {
    if (!order?.items) return parseFloat((totalAmount as string) || '0');
    return order.items.reduce((total: number, item: any) => total + Number((item as any).total_price || 0), 0);
  };
  ...
  const currentStatus = order ? ((orderStatuses as Record<string, any>)[order.status] || orderStatuses.pending) : null;
    ...
    <div className="min-h-screen bg-gradient-to-br from-sangeet-neutral-950 via-sangeet-neutral-900 to-sangeet-neutral-950">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <ReviewModal
          ...
          order={reviewModal.order as any}
          customerName={order?.customer_name}
          tableNumber={order?.table_number}
          onReviewSubmitted={() => {
            if (reviewModal.order) {
              setReviewedOrders(prev => new Set(Array.from(prev).concat([(reviewModal.order as any)!.id])));
            }
