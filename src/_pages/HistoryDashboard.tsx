...
const HistoryDashboard = () => {
  ...
  const { data = [] as any[], isLoading: loading } = useQuery<any[]>({
    ...
    queryFn: (() => {
      const queryFilters = { archived: true, startDate: filters.startDate, endDate: filters.endDate };
  ...
  const handleRestoreOrder = async (orderId: string | number) => {
    try {
      ...
      toast.success('Order successfully restored to Active Management');
              ...
              <div className="p-6 overflow-y-auto">
                ...
                <div className="space-y-3">
                  {selectedOrder.items && selectedOrder.items.length > 0 ? (
                    selectedOrder.items.map((cartItem: any, idx: any) => (
                      <div key={idx} className="flex justify-between items-center p-3 bg-sangeet-neutral-800/50 rounded-lg">
                        <div>
                          <p className="text-white font-medium">{cartItem.quantity}x {cartItem.menu_item_name || 'Item'}</p>
                          {cartItem.notes && <p className="text-sm text-sangeet-neutral-400 mt-1">Note: {cartItem.notes}</p>}
                        </div>
                        <p className="text-white font-medium">{formatCurrency(cartItem.total_price)}</p>
                      </div>
                    ))
                  ) : (
