...
(async () => {
  ...
  const browser = await puppeteer.launch({ headless: 'new', defaultViewport: { width: 1280, height: 800 }, args: ['--no-sandbox', '--disable-setuid-sandbox'] });
  ...
  try {
    ...
    await page.evaluate(() => {
      const btns = Array.from(document.querySelectorAll('button'));
      const viewCartBtns = btns.filter(b => b.innerText.includes('Item') || b.innerText.includes('View cart'));
